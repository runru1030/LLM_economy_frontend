import {
  makeBaseClientString,
  makeDeclareString,
  makeUseMutationString,
  makeUseQueryString,
} from "./make.js";
...
program
  ...
  .action(async (str: { input?: string; output?: string }) => {
    if (!str.input) {
    ...
    declareString = declareString.replace(
      /content\?: never;/gi,
      `content: { "application/json": Record<string, undefined> };`,
    );
