...
export type CreateSummaryListV1Data = _FetchResponse["data"];
export type CreateSummaryListV1Error = _FetchResponse["error"];
export type CreateSummaryListV1Params = _FetchRequest["params"];
export type CreateSummaryListV1Body = _FetchRequest["body"];

export const createSummaryListV1 = async (options: FetchOptions<Api>) => {
  return await client.POST(ENDPOINT, options);
};

const errorTypeGuard = (x: unknown, y: unknown): x is CreateSummaryListV1Data => {
  return !y;
};

export function useCreateSummaryListV1Mutation(
  ...
  return useMutation({
    ...
    mutationFn: async ({ params, body }) => {
      const { data, error, response } = await createSummaryListV1({
        params,
        body,
        ...fetchOptions,
      });
